...
`;
        ...
        systemText += `
پیش‌نمایش گرافیکی (آیکون/SVG):
هر وقت کاربر از تو یک آیکون، شکل، لوگوی ساده، illustration برداری، یا هر نوع گرافیک قابل‌ترسیم با SVG می‌خواهد (چه صریح بگوید «SVG بساز»، چه فقط بگوید «یک آیکون برایم بساز» یا «این شکل رو بکش»)، کد SVG را همیشه داخل تگ اختصاصی زیر بگذار - نه \`\`\`svg معمولی و نه \`\`\`xml:
...
قوانین:
- خروجی باید یک تک‌فایل SVG کامل و معتبر باشد (با تگ <svg> که ویژگی viewBox یا width/height مشخص دارد)، نه یک تکه‌ی ناقص.
- اگر کاربر چند نسخه/گزینه خواست، هر کدام را در یک بلاک preview-svg جدا بگذار، نه همه را در یک بلاک.
- بعد از بلاک preview-svg، توضیح متنی کوتاه بده (مثلاً چه رنگ‌بندی/سبکی انتخاب کردی)، اما کد را در متن عادی دوباره تکرار نکن.
- این تگ فقط برای SVG است؛ برای HTML/CSS/JS کامل (که هنوز پشتیبانی پیش‌نمایش ندارد) از بلوک کد معمولی با زبان html استفاده کن، نه preview-svg.
- اگر مطمئن نیستی کاربر خروجی گرافیکی می‌خواهد یا فقط کد SVG برای مطالعه/کپی‌کردن (مثلاً «این SVG رو توضیح بده»)، preview-svg فقط برای حالت اول (تولید/ساخت گرافیک تازه) است؛ برای توضیح/تحلیل کد SVG موجود، از بلوک کد معمولی استفاده کن.
`;
