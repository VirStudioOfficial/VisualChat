...
`;
        ...
        // FEATURE (پیشنهاد سؤال بعدی / quick-reply): وقتی گفتگو به نقطه‌ای
        // طبیعی برای ادامه رسیده (نه بعد از هر پیام)، مدل خودش تشخیص
        // می‌دهد و حداکثر ۲ پیشنهاد کوتاه تولید می‌کند که کاربر با کلیک
        // رویشان، متن کامل را به‌جای تایپ کردن ارسال می‌کند.
        // FIX: نسخه‌ی قبلی می‌گفت «اگر پاسخ خودش یک سؤال از کاربر است این
        // بلاک را نساز» - اما چون مدل تقریباً همیشه پاسخش را با یک سؤال
        // متقابل تمام می‌کند (عادت مکالمه‌ای)، این دو قانون با هم تداخل
        // داشتند و در عمل بلاک هرگز ساخته نمی‌شد (در تست واقعی کاربر بعد
        // از دیپلوی تأیید شد). راه‌حل: به‌جای حذف/تضعیف قانون، خودِ سؤالِ
        // متقابل باید به داخل پیشنهادها منتقل شود - یعنی وقتی می‌خواهی
        // ادامه‌ی گفتگو را از کاربر بپرسی، آن سؤال را در متن آزاد پاسخ
        // تکرار نکن، بلکه یک جمله‌ی خیلی کوتاه بالای بلاک بگذار و خودِ
        // گزینه‌های جواب را در آرایه‌ی زیر بگذار (کارت‌ها همان سؤالند، نه
        // چیز اضافه‌ای کنار سؤال).
        systemText += `
پیشنهاد سؤال بعدی (quick-reply):
اگر و فقط اگر ادامه‌ی طبیعی همین گفتگو ۱ یا ۲ سؤال/درخواست محتمل و مشخص دارد (نه برای هر پیام - اکثر پاسخ‌ها اصلاً نباید این بلاک را داشته باشند، فقط وقتی واقعاً به یک نقطه‌ی تصمیم یا شاخه‌ی بعدی طبیعی رسیده‌ای)، بعد از پاسخ متنی عادی‌ات یک بلاک اضافه‌ی نامرئی از این نوع اضافه کن (کاربر این بلاک را نمی‌بیند، فقط کلاینت آن را به دکمه تبدیل می‌کند):
...
- مهمِ خیلی مهم درباره‌ی سؤال متقابل: اگر می‌خواهی از کاربر بپرسی می‌خواهد در چه مسیری ادامه دهد (که معمولاً همین‌جاست که این بلاک را می‌سازی)، آن سؤال را در متن آزاد پاسخ به‌صورت کامل و باز ننویس (مثلاً منشین یک پاراگراف با «حالا خودت چی می‌خوای؟» یا «کدومو ترجیح می‌دی؟» تمام نکن) - چون این باعث تکرار همان چیزی می‌شود که کارت‌ها دارند نشانش می‌دهند. به‌جایش در متن آزاد فقط یک جمله‌ی خیلی کوتاه و کلی بگذار (مثل «دو تا مسیر برات گذاشتم، هرکدومو خواستی انتخاب کن 👇» یا حتی هیچی اضافه نکن)، و خودِ گزینه‌های واقعی/دقیق فقط در label/text کارت‌ها باشند.
- اگر گفتگو در جایی است که ادامه‌ی طبیعی روشن نیست، این بلاک را اصلاً اضافه نکن (نه اینکه سؤال را در متن بنویسی و بلاک را هم نسازی).
`;
