...
//   {"type":"done"}ش// api/voice.js — ویس «نوبتی» روی Vercel (بدون WebSocket، فقط HTTP streaming)
//
// چرا این‌طوری؟ Vercel WebSocket سرور نگه نمی‌داره و Render/Deno/Cloudflare
// از ایران مشکل دارن. این endpoint هر «نوبت» صحبت رو جدا می‌گیره:
//
...
//   {"type":"error","message":"..."}
//
// متغیر محیطی: GEMINI_API_KEY (همون که سایت داره)
// اختیاری:     VOICE_SECRET  (اگه ست شه، اپ باید هدر x-voice-secret بفرسته)
...
export default async function handler(req) {
  ...
  if (!key) return json({ error: "GEMINI_API_KEY missing on server" }, 500);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const speak = (sentence) => {
        ...
        ttsChain = ttsChain.then(async () => {
          try {
            const pcm = await tts(key, t, voiceName);
            if (pcm) send({ type: "audio", data: pcm });
          } catch (e) {
            send({ type: "error", message: "tts: " + (e?.message || e) });
          }
      ...
      try {
        const r = await fetch(`${API}/${TEXT_MODEL}:streamGenerateContent?alt=sse`, {
          method: "POST",
          headers: { "content-type": "application/json", "x-goog-api-key": key },
          body: JSON.stringify({
            systemInstruction: { parts: [{ text: sys }] },
...
async function tts(key, text, voiceName) {
